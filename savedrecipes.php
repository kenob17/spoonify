...
$info = $db->get('saved_recipes');
$saved_recipes = Array();

foreach($info as $item){
  array_push($saved_recipes, $item['food_id']);
}
...
<html lang="en">
  ...
  <nav
    ...
    <section class="mt-20 text-center">
      <p class="font-bold text-2xl">Saved Recipes</p>
    </section>
  ...
  <script>
    var data = <?php echo json_encode($saved_recipes); ?>;
    var key = <?php echo $key; ?>;

    data.forEach(getrecipe);

    function getrecipe(id) {
      $.ajax({
        url: "https://api.spoonacular.com/recipes/"+ id + "/information?apiKey=9a73fd73b6214058aa415ecb28a85973&includeNutrition=false",
        success: function (res) {
          ...
          link.href = "recipe.php?details=" + res.sourceUrl + "&key=" + key;       
          ...
          author.innerHTML = "Author: " + res.author;
